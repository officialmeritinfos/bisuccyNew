<template>
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <PageTitle :title="$t('cryptoDeposits')" />
    </div>
...
const cryptoDepositList = computed(() => depositStore.cryptoDepositsList);
const tableRef = ref();
const tabulator = ref();
...
watch(
    computed(() => cryptoDepositList.value),
    () => {
        initTabulator();
    }
);
...
onMounted(async () => {
    await depositStore.getCryptoDepositList();
